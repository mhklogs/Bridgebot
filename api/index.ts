...
app.post("/api/migrate", async (req, res) => {
  ...
  if (ai && hasValidKey) {
    try {
      ...
      console.error("Gemini API error (migrate):", JSON.stringify({ message: error.message, name: error.name, status: error.status, code: error.code }));
    }
  }
...
app.post("/api/refine", async (req, res) => {
  ...
  if (ai && hasValidKey) {
    try {
      ...
      console.error("Gemini API error (refine):", JSON.stringify({ message: error.message, name: error.name, status: error.status, code: error.code }));
    }
  }
...
if (fs.existsSync(distPath)) {
  ...
  app.get("*", (req, res) => {
    ...
    if (fs.existsSync(indexPath)) {
      ...
      res.status(200).send("BridgeBot API is running. Frontend not found at: " + distPath);
    }
  });
}
